...
- Grist CRM sync now also runs as a **background worker** (in-process scheduler started from `instrumentation.ts`, like the calendar sync), pulling the CRM on a schedule (`GRIST_SYNC_TICK_INTERVAL_MS`, default 30 min) in addition to the manual button. The Statistics page gained a "Contacts & Grist CRM sync worker" section: contact counters (people, CRM profiles, with company, with photo) plus a live worker panel with a countdown to the next sync, time-since-last-run, last-run counters, and an activity feed of recent sync runs. The calendar "Live sync" card is renamed "Calendar sync worker"
- Grist CRM integration: one-way pull of the Grist `Connections`/`Companies` tables into the contact profile overlay (`POST /api/grist/sync`), configured via `GRIST_*` env vars and shown read-only under Settings → Accounts with a Sync now button. Grist is authoritative for the fields it owns (name, company, title, phone, notes, photo) — each sync overwrites them to match Grist, clearing a field when its Grist cell is empty. Profile photos are downloaded into `data/contact-photos/` and served locally from `/api/contact-photos/:file` instead of hot-linking the CRM server
- Contacts page also lists CRM-only contacts (imported from Grist but not yet met) with a "No meetings yet" marker, so people appear as soon as you add their email to the CRM and link up automatically once you share an event with that address
...
- Unified loading UI: a global activity indicator in the top navigation bar shows a live elapsed-seconds counter while requests are in flight ("Loading events… 3s"), an idle state otherwise, and an expandable panel listing each live request (method, path, elapsed time) as a lightweight built-in network monitor. It also surfaces a prominent, dismissible amber warning when a load can't show everything (e.g. a truncated events window), so events are never silently dropped. Backed by a reusable `<LoadingIndicator>` primitive with an always-on timer and optional stat slots, and a global activity store with opt-in `trackedFetch`/`withActivity` helpers
...
- Calendar now loads events incrementally per visible window instead of pulling the entire archive up front: navigating a view fetches only that range via the windowed API, already-seen ranges are served instantly from an in-memory cache, and the previous/next period is prefetched in the background so navigation feels instant. Adding, editing, deleting, or syncing an event (and toggling a calendar on/off) invalidates the cache and refetches the current window. Each fetch reports into the global activity indicator, and a window that hits the server row cap raises a dismissible "Some events not shown — narrow your view" warning so events are never silently dropped

### Fixed

- Auto-schedule now ignores deleted/archived events when checking availability. Because deletions are preserved as `status:"cancelled"` in the archival mirror, the scheduler's calendar-conflict query was counting soft-deleted events (e.g. a cancelled recurring "work" block) as busy time, which could fill the entire work window and leave no slots — so no tasks got scheduled. The availability query now excludes cancelled events (NULL-status events are still treated as busy)
